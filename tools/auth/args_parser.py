...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Unauthorized Access Vulnerability Scanner"
    )
    ...
    parser.add_argument(
        "-o", "--output", help="Output filename for results (CSV or JSON format)"
    )
    ...
    parser.add_argument(
        "-s",
        "--services",
        help="Comma-separated list of services to scan (e.g., MongoDB,Redis). "
    ...
    parser.add_argument(
        "--list-services",
        action="store_true",
        help="List all available services for scanning.",
    )
